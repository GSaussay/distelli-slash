...
function processQuery(slackToken, slackTeamId, query, username, callback) {
  ...
  //List builds
    ...
    request(endPoint + '/builds?apiToken=' + secrets.users[username] + '&max_results=50&order=desc', function (error, response, body) {
      console.log(error);
      if(!error && response.statusCode == 200){
        var contents = JSON.parse(body);
        var returnData = [];
  ...
  //List servers
    ...
    request(endPoint + '/servers?apiToken=' + secrets.users[username] + '&max_results=50', function (error, response, body) {
      if(!error && response.statusCode == 200){
        var contents = JSON.parse(body);
        var returnData = [];
  ...
  //List envs for app [appName]
    ...
    var appName = queryArray[4];
     request(endPoint + '/apps/' + appName + '/envs?apiToken=' + secrets.users[username] + '&max_results=25', function (error, response, body) {
      if(!error && response.statusCode == 200) {
        var contents = JSON.parse(body);
        var returnData = [];
        for(var i =0; i < contents.envs.length; i++) {
  ...
  //List servers for env [envName]
    ...
    var envName = queryArray[4];
     request(endPoint + '/envs/' + envName + '/servers?apiToken=' + secrets.users[username] + '&max_results=25', function (error, response, body) {
        if(!error && response.statusCode == 200){
          var contents = JSON.parse(body);
          var returnData = [];
          for(var i =0; i < contents.servers.length; i++){
  ...
  //Create app [appName]
    ...
    var appName = queryArray[2];
     request.put({headers: {'content-type' : 'application/json'}, url: endPoint + '/apps/' + appName + '?apiToken=' + secrets.users[username] + ''}, function(error, response, body) {
        if(!error && response.statusCode == 200){
          var contents = JSON.parse(body);
          console.log(body);
   ...
   //Create env [envName] for app
    ...
    request.put({headers: {'content-type' : 'application/json'}, url: endPoint + '/apps/'+ appName + '/envs/' + envName + '?apiToken=' + secrets.users[username]}, function (error, response, body) {
      if(!error && response.statusCode == 200){
        var contents = JSON.parse(body);
  ...
  //Restart env [envName]
    ...
    var appName = queryArray[5];
     ...
     request.put(endPoint + '/envs/' + envName + '/restart?apiToken=' + secrets.users[username], function (error, response, body) {
        if(!error && response.statusCode == 200){
          var contents = JSON.parse(body);
  ...
  //List releases for app [appName]
    ...
    var queryArray = query.split(" ");
    var appName = queryArray[4];
     request(endPoint + '/apps/' + appName + '/releases?apiToken=' + secrets.users[username] + '&max_results=25&order=desc', function (error, response, body) {
      if(!error && response.statusCode == 200){
        var contents = JSON.parse(body);
        var returnData = [];
      ...
      if(!error && response.statusCode == 200){
        ...
        request(endPoint + '/apps/' + appName + '?apiToken=' + secrets.users[username], function (error, response, body){
          if(!error && response.statusCode == 200){
            ...
            request.post({headers: {'content-type' : 'application/json'}, url: endPoint + '/envs/' + envName + '/deploy' + '?apiToken='+ secrets.users[username] + '',
